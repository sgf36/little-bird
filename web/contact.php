<?php
/**
 * Contact form handler for wren.spencerfields.com.
 *
...
const MAIL_FROM          = 'noreply@wren.spencerfields.com';
...
{
    $path = sys_get_temp_dir() . '/wren-contact-' . hash('sha256', $ip) . '.txt';
    $now = time();
    $stamps = [];
...
<div class="wrap">
  <div class="topbar">
    <a class="brand" href="index.html">Wren</a>
    <nav>
  ...
  <section>
    <h1>{$heading}</h1>
    <p class="{$cls}">{$body}</p>
    <p><a href="support.html">Back to support</a></p>
  </section>
</div>
</body>
</html>
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') !== 'POST') {
    header('Location: support.html', true, 302);
    exit;
}
...
$subject = header_safe('[Wren] ' . $topic . ' — ' . $name);

$body = "A message was sent from the Wren contact form.\n\n"
      . "Name:    {$name}\n"
      . "Email:   {$email}\n"
      . "Topic:   {$topic}\n"
      . "IP:      {$ip}\n"
...
$headers = implode("\r\n", [
    ...
    'X-Mailer: wren.spencerfields.com',
]);
...
if (!$sent) {
    error_log('[wren contact] Worker relay failed, falling back to mail(): ' . $relayError);
