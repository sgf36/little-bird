...
def shot_prices():
    """The real storefront price per shot language, or {} with a warning.
    ...
    if not path.exists():
        say(f"! {path.name} is missing — the paywall scene will show the app's "
            f"fallback dollar price. Run `python store/iap_prices.py --write`.")
        return {}
    try:
        ...
        return {}
    say(f"prices for {len(prices)} locales from {path.name}")
    return prices


def name_scene(app_tmp, scene, price=None):
    """Write the scene file, and prove it was written by reading it back."""
    if app_tmp is None:
        return False
    target = app_tmp / SCENE_FILE
    body = scene if price is None else f"{scene}\nprice={price}"
    try:
    ...
    for scene in SCENES:
        ...
        for attempt in range(3):
            run("xcrun", "simctl", "terminate", udid, BUNDLE, check=False,
                quiet=True)
            named = name_scene(app_tmp, scene, price)
